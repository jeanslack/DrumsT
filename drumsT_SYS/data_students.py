...
class Students_Id(object):
    ...
    def first_start(self, path, name, year):
        ...
        """
        try:
            conn = sqlite3.connect('%s/%s/%s/students.drtDB' % (path,name,year))
            #conn.row_factory = sqlite3.Row
            cursor = conn.cursor()
            
            cursor.execute("""CREATE TABLE students 
                           (name text, address text, birth_dates text, 
                           phone text, joined_date text, level_course text)"""
                           )
            conn.commit()
            conn.close()
...
#def create_new(path_db):
    ...
    def insert(self,name,address,birth_dates,phone,date,level,path_db):
        """
